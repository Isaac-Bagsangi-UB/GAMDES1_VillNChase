...
Objectives - Escape the facility by reaching the exit before time runs out. - Escape with at least a heart remaining - There may be more objectives as difficulty increases



Rules - The player can only carry one item at a time. - Locked doors can only be used when unlocked with a key or mechanism



Procedures - Choose a map and a difficulty - Move and explore, find and use items, interact with mechanisms, avoid or deal with enemies and traps, find the exit - Unlock the exit and Escape



Resources - Items such as Potions, Weapons, Repair Tools, Keys - Hearts - Time remaining before the game ends



Conflict - Heroes and other villains can damage the player or cast abilities - Traps can damage, stun, or slow. - The player should reach the exit before time runs out.
...
4.2 Loop Walkthrough

The core gameplay loop begins with Action, where the player performs an action based on the current situation in the facility. This may include moving, jumping, avoiding a trap, dealing with an enemy, using an item, or interacting with a mechanism.



The player action then produces Feedback from the game. Depending on the action, the player may lose a heart when a player bumps to an enemy or triggers a trap, or an item or interactive mechanism may trigger its intended action or effect when used.



The player then receives a Reward or Progress from successfully responding to the situation. This may include gaining an item, gaining access to a previously blocked area, gaining an achievement, or escaping the current facility and advancing to the next level.



The resulting progress provides Motivation for the player to continue. The player may decide to try a different approach, explore another route, play a different map, or try another level of difficulty which results in restarting the core gameplay loop.



One complete pass through the core gameplay loop is expected to take approximately 10-30 seconds, depending on the player’s action, and the player’s decision-making speed.
